...
In the Welsh Care Records Service (WCRS), each new version of a document was represented as a separate record, each with its own unique `wcrsDocumentId`. These records were linked together only through a shared `wcrsSupersessionSetId`, which identified the **supersession set** to which all versions belonged. 
...
All versions are expressed as history versions of the same FHIR resource, not as separate ones. 
