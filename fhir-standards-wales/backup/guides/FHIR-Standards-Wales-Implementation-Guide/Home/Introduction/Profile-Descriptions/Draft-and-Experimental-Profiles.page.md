...
<br>
#### Experimental
Experimental status is an additional flag appended to Draft status. It is used when resources are developed for exploratory purposes such as prototyping. Such resources are not intended for use, nor will they progress to a non-experimental state. Note that this flag has been inappropriately used in production versions of this implementation guide in the past. All of the incorrect flagging of resources as experimental has now been removed. In future, experimental resources may only appear in versions of this implementation guide on development or test endpoints.
<br>